...
- **Telegram-бот** (`botsrc/`): лёгкий клиент без зависимостей (long-polling, без VPS)
  ...
  - Приём сообщений из Telegram → в чат DeepSeek (нативный Enter)
  - Команда `/todos` — список текущих задач активного окна из Telegram
  - Уведомление «🎉 Все задачи выполнены» со списком задач, когда весь список todo становится `completed` (однократно, с повторным срабатыванием после изменения списка)
  - Настройки в **Настройки → Cookie Code → Telegram-бот** (токен/chat ID/флаги)
- **@-упоминание папок** в поле ввода: теперь в автодополнении по `@` показываются и файлы, и папки проекта (папки сверху, с иконкой 📁); исключаются `node_modules`, `.git`, `dist`, `build` и т.п.
- Кнопка экстренной остановки (Kill) активных дочерних процессов в плашке «Выполняется»
- Декорирование tool-блоков: горячий путь обходит все ответы с необработанными блоками (ленивая догрузка истории DeepSeek)
- RGB-переливание приветственного текста на стартовой странице
- Перенос дизайна страницы настроек в оверлей (карточки, стекло, акценты)

### Fixed
- Ненулевой код выхода `bash`/`pwsh` → `❌` в уведомлении Telegram
- Текст ошибки «Слишком частые сообщения» (DeepSeek rate limit) — красный жирный
- Панель/таб Todo больше не всплывает после вызова сторонних инструментов: событие `todo-updated` из `execute-js` шлётся только при реальном изменении списка, а флаг ручного закрытия в рендере сбрасывается только при изменении состава задач
- Устранён краш «Invalid or unexpected token» в `botsrc/index.js` из-за неверного экранирования `\n` в шаблонных строках
