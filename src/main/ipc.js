...
function finishTask(senderId, token) {
  if (activeTaskTokens.get(senderId) === token)
    activeTaskTokens.delete(senderId);
}
...
function maybeNotifyAllDone(senderId) {
  try {
    ...
    if (done === todos.length) {
      ...
      try {
        require("../../botsrc").notifyAllDone(todos);
      } catch (_) {}
    } else {
      _todoAllDoneNotified.delete(senderId);
    }
...
async function insertImageToChat(sender, filePath, caption, send) {
  try {
    ...
    if (img.isEmpty()) {
      return {
        success: false,
        error: `Не удалось загрузить изображение: ${filePath}`,
      };
    }
  ...
  return new Promise((resolve) => {
    ...
    exec(cmd, { timeout: 5000 }, (error, _stdout, stderr) => {
      if (error) {
        resolve({
          success: false,
          error: (stderr || error.message || "code не найден").trim(),
        });
        return;
      }
...
function registerIpcHandlers() {
  ...
  ipcMain.handle("set-plan-mode", async (event, { enabled } = {}) => {
    ...
    return {
      success: true,
      planMode: planMode.isPlanMode(event.sender.id, sessionId),
      sessionId,
    };
  });
  ...
  ipcMain.on(
    ...
    (event, { requestId, answers, canceled } = {}) => {
      if (!requestId) return;
      const key = `${event.sender.id}:${requestId}`;
  ...
  ipcMain.handle("list-project-files", async (event, { query = "" } = {}) => {
    ...
    try {
      ...
      // @-поиск должен игнорировать регистр: --glob в ripgrep регистрозависим,
      // а --iglob — нет. buildGlobArgs не трогаем (glob-инструмент остаётся как был).
      const args = buildGlobArgs({ pattern }).map((a) =>
        a.startsWith("--glob=") ? "--iglob=" + a.slice("--glob=".length) : a,
      );
      ...
      (function walk(absDir, relDir) {
        let entries;
        try {
          entries = require("fs").readdirSync(absDir, { withFileTypes: true });
        } catch (_) {
          return;
        }
        for (const e of entries) {
          ...
          if (safeQuery && !rel.includes(safeQuery)) {
            /* всё равно заходим глубже */
          }
  ...
  ipcMain.handle("navigate-session", async (event, { sessionId }) => {
    ...
    try {
      ...
      if (
        provider &&
        typeof provider.sessionUrlBase === "string" &&
        provider.sessionUrlBase
      ) {
  ...
  ipcMain.handle(
    "execute-tool",
    async (event, { toolName, params, callId }) => {
      const ctx = windowState.getContextByWebContents(event.sender);
      const store = ctx ? ctx.sessionStore : null;
      const selectedDir = store ? store.state.selectedProjectDir : null;
      ...
      try {
        ...
        if (
          ...
          try {
            event.sender.send("todo-updated", {
              todos: todoStore.getList(event.sender.id),
            });
          } catch (_) {}
        ...
        try {
          ...
          require("../../botsrc").notifyToolResult(toolName, ok, {
            args: params,
            preview,
          });
        } catch (_) {}
        ...
        try {
          require("../../botsrc").notifyToolResult(
            toolName,
            false,
            err.message,
          );
  ...
  ipcMain.handle(
    "chat-insert-image",
    async (event, { filePath, caption, send } = {}) => {
      return insertImageToChat(
        event.sender,
        filePath,
        String(caption || ""),
        send !== false,
  ...
  ipcMain.handle("show-ai-notification", async (event) => {
    try {
      ...
      if (win && !win.isDestroyed()) {
        ...
        const lang =
          settingsStore.getSetting("language") === "en" ? "en" : "ru";
        const notifText = {
        ...
        notification.on("click", () => {
          try {
            ...
            console.error(
              "[Cookie Code] notification click focus error:",
              err.message,
            );
          }
        });
  ...
  ipcMain.handle("cuckoo-chat-export", async (_event, payload) => {
    try {
      console.log(
        "[Cookie Code] chat-export: получен запрос, format =",
        payload && payload.format,
        ", html.length =",
        payload && payload.html && payload.html.length,
      );
      const result = await chatExport.exportChat(payload || {});
      console.log(
        "[Cookie Code] chat-export: результат =",
  ...
  ipcMain.handle("todo-clear", async (event) => {
    try {
      ...
      try {
        event.sender.send("todo-updated", { todos: [] });
      } catch (_) {}
      return { success: true };
    } catch (err) {
      return { success: false, error: err.message };
  ...
  ipcMain.handle("cuckoo-backgrounds-list", async () => {
    try {
      ...
      console.error(
        "[Cookie Code] 读取 пользовательских фонов失败:",
        err.message,
      );
      return {
        success: false,
        error: err.message,
        dir: getCustomBackgroundsDir(),
        backgrounds: [],
      };
    }
  });
  ...
  ipcMain.handle("get-file-icons", async () => {
    try {
      const fs = require("fs");
      ...
      const icons = {}; // fileName -> svg
      const getIconName = (defName) => {
        const def =
          manifest.iconDefinitions && manifest.iconDefinitions[defName];
        if (!def || !def.iconPath) return null;
        const fileName = path.basename(def.iconPath);
        if (icons[fileName] === undefined) {
          try {
            let svg = fs.readFileSync(path.join(iconsDir, fileName), "utf8");
            // Класс — чтобы отличить Material-иконку от fallback в CSS.
            svg = svg.replace(
              "<svg",
              '<svg class="cuckoo-file-chip__material"',
            );
            icons[fileName] = svg;
          } catch (_) {
            icons[fileName] = null;
        ...
        return icons[fileName] ? fileName : null;
      };
      const fileExtensions = {};
      for (const [ext, def] of Object.entries(manifest.fileExtensions || {})) {
        const n = getIconName(def);
        if (n) fileExtensions[ext] = n;
      }
      const fileNames = {};
      for (const [nm, def] of Object.entries(manifest.fileNames || {})) {
        const n = getIconName(def);
        if (n) fileNames[nm] = n;
      }
      return { success: true, data: { icons, fileExtensions, fileNames } };
    } catch (err) {
      console.error("[Cookie Code] get-file-icons failed:", err.message);
      return { success: false, error: err.message };
    }
  });
}
