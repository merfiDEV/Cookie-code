...
try {
  ...
  console.error(
    "[Cookie Code] Не удалось прочитать логотип DeepSeek:",
    err.message,
  );
}
...
function showConfirmDialog(text, options) {
  ...
  return new Promise((resolve) => {
    ...
    if (showCancel) {
      dialog
        .querySelector("#cuckoo-confirm-cancel")
    ...
    if (res && res.success) {
      showToast(
        res.count > 0
          ...
          : t("overlay.task.stopped"),
        2500,
      );
      setTaskStatus(false);
    } else {
      showToast(
        t("overlay.task.killError") +
...
function renderHistory() {
  ...
  if (commandHistory.length === 0) {
    list.innerHTML =
      '<div style="color:#666;font-size:12px;font-style:italic;padding:8px 0;">暂无记录</div>';
    return;
  }
    ...
    el.addEventListener("click", () => {
      ...
      if (entry) {
        ...
        if (entry.output && resultSection) {
          ...
          if (resultStatus) {
            resultStatus.textContent = entry.canceled
              ? "⏹ 已忽略"
              : entry.success
                ? "✅ 执行成功"
                : "❌ 执行失败";
          ...
          if (resultOutput)
            resultOutput.textContent = entry.output || "(无输出)";
        }
        showOverlay();
      }
    });
...
function updateHomeMode() {
  ...
  const isHome =
    provider && provider.homeUrlPattern
      ? provider.homeUrlPattern.test(url)
      : false;
  const overlay = document.getElementById("cuckoo-overlay");
  if (overlay) {
    if (isHome) {
...
/**
 * 显示首次使用提示浮窗（居中）
 * Не показывается, если проект уже выбран или окно закрыто вручную в этой сессии.
 */
function showFirstTimeDialog() {
  if (state.currentProjectDir || state.firstTimeDialogDismissed) {
    hideFirstTimeDialog();
    return;
  }
  const dialog = document.getElementById("cuckoo-first-time-dialog");
  if (!dialog) return;
  // Уже видим — ничего не делаем (иначе анимация будет перезапускаться).
  if (!dialog.classList.contains("cuckoo-hidden")) return;
  dialog.classList.remove("cuckoo-closing");
  dialog.classList.add("cuckoo-enter");
  dialog.classList.remove("cuckoo-hidden");
}

/**
 * 隐藏首次使用提示浮窗 с плавной анимацией исчезновения.
 * @param {boolean} [permanent] - если true, окно больше не показывается в этой сессии
 */
function hideFirstTimeDialog(permanent) {
  const dialog = document.getElementById("cuckoo-first-time-dialog");
  if (!dialog) return;
  if (permanent) state.firstTimeDialogDismissed = true;
  if (dialog.classList.contains("cuckoo-hidden")) return;
  if (dialog.classList.contains("cuckoo-closing")) return;
  dialog.classList.add("cuckoo-closing");
  setTimeout(() => {
    dialog.classList.add("cuckoo-hidden");
    dialog.classList.remove("cuckoo-closing");
    dialog.classList.remove("cuckoo-enter");
  }, 220);
}
