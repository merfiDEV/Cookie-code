...
function isJsonBalanced(str) {
  ...
  if (
    !/toolName|"tool"|file_|json复制|```/.test(trimmed) &&
...
/**
 * Идёт ли прямо сейчас генерация ответа ИИ.
 * Прямой DOM-сигнал: активная (не disabled) круглая primary-кнопка «стоп»
 * в composer. В отличие от isAIResponseComplete() не зависит от трактовки
 * завершённости и не срабатывает в пустом диалоге.
 */
function isGenerating() {
  try {
    const stopBtn = document.querySelector(
      ".ds-button.ds-button--primary.ds-button--filled.ds-button--circle" +
        ".ds-button--m.ds-button--icon-relative-m:not(.ds-button--disabled)",
    );
    return !!stopBtn;
  } catch (_) {
    return false;
...
async function executeJsBlocksWithRetry(initialBlocks, markdown, force) {
  ...
  for (let attempt = 0; attempt <= MAX_JS_RETRY; attempt++) {
    ...
    const hasIncompleteFailure = results.some(
      (item) =>
        ...
        item.result &&
        !item.result.success &&
        looksLikeIncompleteCodeError(item.result.error),
    );
    ...
    if (attempt < MAX_JS_RETRY) {
      console.log(
        "[" +
          ...
          "] [Cookie Code] ⏳ 代码不完整，等待 1 秒后重新获取并重试（" +
          (attempt + 1) +
          "/" +
          MAX_JS_RETRY +
          "）...",
  ...
  if (results.length > 0) {
    ...
    for (const item of results) {
      try {
        toolResultInline.markToolBlockResult(item.code, item.result);
      } catch (_) {}
      ...
      try {
        ...
        if (r) {
          if (Array.isArray(r.stats)) {
            ...
            added = Number(r.stats.added) || 0;
            removed = Number(r.stats.removed) || 0;
          } else if (r.data && r.data.stats) {
            added = Number(r.data.stats.added) || 0;
            removed = Number(r.data.stats.removed) || 0;
          }
        }
    ...
    try {
      const msgEl =
        (markdown && typeof markdown.closest === "function"
          ? markdown.closest(".ds-message")
          : null) || responseMeta.findLatestAIMessage();
  ...
  try {
    ...
    if (p && typeof p.catch === "function") {
      p.catch((err) => {
        ...
        try {
          console.warn(
            "[Cookie Code][safe:observer.processLatestAIResponse] " + msg,
          );
        } catch (_) {}
      });
    }
...
async function processLatestAIResponseInner(retryCount = 0, force = false) {
  ...
  if (
    providerForUser &&
    typeof providerForUser.isUserMessage === "function" &&
    providerForUser.isUserMessage(lastMessage)
  ) {
  ...
  console.log(
    "[DEBUG][processLatest] lastMessage=" +
      (lastMessage.className || lastMessage.tagName) +
      " markdown=" +
      (markdown.className || markdown.tagName) +
      " jsBlocks=" +
      jsBlocks.length +
      " force=" +
      force +
  ...
  if (jsBlocks.length > 0) {
    ...
    if (!rec || rec.snapshot !== snapshot || rec.blocksSig !== blocksSig) {
      ...
      console.log(
        "[Cookie Code] ⏳ 检测到 JS 工具代码块，流式渲染中，等待稳定...",
      );
    ...
    processedMessages.add(lastMessage);
    console.log(
      "[Cookie Code] ✅ 代码块稳定，检测到 JS 工具代码块（" +
        jsBlocks.length +
        " 个），开始执行",
    );
  ...
  if (!force) {
    ...
    if (!rec || rec.snapshot !== snapshot) {
      ...
      ensureStabilityTimer();
      console.log(
        "[Cookie Code] ⏳ 文本内容渲染中，等待稳定（防流式中途漏检）...",
      );
      return;
    }
    if (now - rec.lastChange < JS_STABILITY_WINDOW) {
      console.log(
        "[Cookie Code] ⏳ 文本内容稳定中（等待 " +
          JS_STABILITY_WINDOW +
          "ms 确认）...",
      );
      return;
  ...
  const looksToolish =
    text.includes(FENCE) ||
    /toolName|"tool"|file_|await\s+(?:read|write|edit|glob|grep|bash|pwsh|todoWrite|deleteFile|webFetch|cityTime|openBrowserWindow|injectJS|readFile|writeFile|editFile)\s*\(/.test(
      text,
    );
  ...
  if (!force && !isJsonBalanced(text)) {
    ...
    console.log(
      "[Cookie Code] ⚠️ JSON 持续不完整（20次重试仍截断），放弃本次处理，当前长度=" +
        text.length,
    );
    // 回传 AI，让它重新完整输出
    sendToolResultToChat(
      { toolName: "未知", callId: "incomplete" },
      {
        success: false,
        error:
          "收到不完整的工具调用 JSON（内容被截断），请重新完整输出工具调用。",
      },
    );
  }
  ...
  if (toolCall) {
    ...
    if (!available) {
      ...
      sendToolResultToChat(toolCall, {
        success: false,
        error:
          "工具 " + toolCall.toolName + " 不存在，可用工具: " + toolNamesList(),
      });
      return;
    }
    console.log(
      "[Cookie Code] ✅ 工具存在: " + toolCall.toolName + ", 开始执行",
    );
    ...
    console.log(
      "[Cookie Code] [XML诊断] text长度=" +
        text.length +
        ", 开头100字符=" +
        JSON.stringify(text.slice(0, 100)),
    );
    console.log(
      "[Cookie Code] [XML诊断] markdown.innerHTML长度=" +
        (markdown.innerHTML || "").length +
        ", 开头200字符=" +
        JSON.stringify((markdown.innerHTML || "").slice(0, 200)),
    );
    console.log(
      "[Cookie Code] [XML诊断] 是否有 pre code 元素=" +
    ...
    if (hasAntmlXml || (hasXmlInvoke && (hasXmlClose || hasXmlParam))) {
      ...
      xmlHintCount++;
      console.log(
        "[Cookie Code] ⚠️ 检测到 XML 格式工具调用（第 " +
          xmlHintCount +
          " 次提示），提示 AI 改用 cuckoo 代码块",
      );
    ...
    if (looksToolish) {
      ...
      console.log(
        "[Cookie Code] ⚠️ 回复疑似工具调用但未被识别（JS 代码块未匹配 / JSON 解析失败）",
      );
      ...
      if (pres.length > 0) {
        for (const p of pres) {
          ...
          const lang =
            providerForLang &&
            typeof providerForLang.getCodeBlockLanguage === "function"
              ? providerForLang.getCodeBlockLanguage(p)
              : "";
          console.log(
            "[Cookie Code] [诊断] 代码块 language=" +
              (lang || "(无)") +
              ", 内容前80字符=" +
              (p.textContent || "").trim().slice(0, 80),
          );
        }
      } else {
      ...
      console.log(
        "[Cookie Code] ℹ️ 正常文本回复，未检测到工具调用（无需处理）",
      );
...
function extractHumanTextFromNode(node) {
  ...
  clone
    .querySelectorAll(
      '.md-code-block, .cuckoo-tool-block, .cuckoo-tool-header, .cuckoo-tool-label, .cuckoo-tool-file, .cuckoo-tool-sep, .cuckoo-tool-icon, .cuckoo-tool-chevron, pre, button, [class*="toolbar"], [class*="copy"], [class*="download"], [class*="code-block"], [class*="lang"]',
    )
    .forEach((el) => el.remove());
...
function startObserver() {
  ...
  const observer = new MutationObserver((mutations) => {
    ...
    try {
      ...
      const latestForMeta =
        metaCandidates.length > 0
          ? metaCandidates[metaCandidates.length - 1]
          : null;
    ...
    if (hasNewContent && !isProcessingResponse) {
      ...
      (async () => {
        try {
          // Индикатор «печатает…» в TG — только пока реально идёт генерация.
          if (isGenerating()) {
            try {
              window.electronAPI.telegramTypingStart();
            } catch (_) {}
          } else {
            try {
              window.electronAPI.telegramTypingStop();
            } catch (_) {}
          }
          if (await isAIResponseComplete()) {
            try {
              responseMeta.finishTimer(responseMeta.findLatestAIMessage());
            } catch (_) {}
  ...
  if (!completionPollTimer) {
    completionPollTimer = setInterval(() => {
      ...
      (async () => {
        try {
          // Страховка: генерация закончилась, но mutation-сигнал потерялся —
          // гасим «печатает…» в TG.
          if (!isGenerating()) {
            try {
              window.electronAPI.telegramTypingStop();
...
function notifyToolCallDetected(toolCall) {
  ...
  if (preview) {
    preview.textContent =
      t("overlay.preview.toolPrefix", { name: toolCall.toolName }) +
      String.fromCharCode(10) +
      t("overlay.preview.params", {
        json: JSON.stringify(toolCall.params, null, 2),
      });
  }
  // 闪烁状态徽章
...
function notifyJsScriptDetected(code) {
  ...
  if (preview) {
    preview.textContent =
      t("overlay.preview.jsPrefix") + String.fromCharCode(10) + code;
  }
...
async function handleJsToolScript(code) {
  ...
  if (!verdict.approved) {
    ...
    return {
      code,
      result: { success: false, denied: true, error: approval.DENIED_JS_ERROR },
    };
  }
  ...
  const callId =
    "js_" + Date.now() + "_" + Math.random().toString(36).substr(2, 6);
  console.log(
    "[Cookie Code] [诊断] 即将执行的代码(JSON转义): " + JSON.stringify(code),
  );
  try {
    ...
    if (result.success) {
      if (resultStatus) {
        ...
        resultStatus.className = hasBadExit
          ? "cuckoo-result-status error"
          : "cuckoo-result-status success";
      }
      if (resultOutput) {
        resultOutput.textContent =
          result.output || t("overlay.output.scriptDone");
      }
      if (hasBadExit) {
        try {
          toolRender.markToolBlockError(
            code,
      ...
      if (resultOutput) {
        resultOutput.textContent =
          result.error || t("overlay.output.unknownError");
      }
      // Помечаем tool-блок в чате как ошибочный
      try {
        toolRender.markToolBlockError(
          code,
          result.error || t("overlay.output.execFailed"),
        );
      } catch (_) {}
    }
    ...
    addHistory({
      id: callId,
      command:
        "[JS] " + truncate(code.split(String.fromCharCode(10))[0] || code, 60),
    ...
    try {
      toolRender.markToolBlockError(
        code,
        "Системная ошибка: " + (err.message || String(err)),
      );
    } catch (_) {}
    return {
      code,
      result: {
        success: false,
        error: t("overlay.output.systemException", {
...
async function handleToolCall(toolCall) {
  ...
  try {
    ...
    if (result.success) {
      if (resultStatus) {
        resultStatus.textContent = t("overlay.status.toolOk", {
          name: toolName,
        });
      ...
      if (resultStatus) {
        resultStatus.textContent = t("overlay.status.toolFailed", {
          name: toolName,
        });
        resultStatus.className = "cuckoo-result-status error";
      }
    ...
    try {
      ...
      if (msgEl)
        responseMeta.recordSingleToolCall(msgEl, toolName, params, result);
    } catch (_) {}
