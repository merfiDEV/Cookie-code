...
try {
  ...
  state = { showProducedFiles: true };
}

let estimateTokens = (s) => (s ? Math.ceil(s.length / 4) : 0);
try {
  ({ estimateTokens } = require("./token-estimator"));
} catch (_) {}

// Иконка файла (Material Icon Theme) для чипов «Затронуто».
let getFileIcon = () => "";
try {
...
try {
  if (state && typeof state.showProducedFiles === "boolean")
    producedEnabled = state.showProducedFiles !== false;
} catch (_) {}
...
function setEnabled(on) {
  ...
  try {
    if (state) state.showProducedFiles = producedEnabled;
  } catch (_) {}
...
function isProducedEnabled() {
  ...
  try {
    if (state && typeof state.showProducedFiles === "boolean")
      return state.showProducedFiles !== false;
  } catch (_) {}
  return true;
}
...
function getMessageKey(messageEl) {
  try {
    ...
    if (!md) return null;
    const norm = String(md.textContent || "")
      .replace(/\s+/g, " ")
      .trim()
      .slice(0, 200);
...
function shortFileName(filePath) {
  try {
    ...
    const filtered =
      parts.length > 0 && /^[A-Za-z]:$/.test(parts[0]) ? parts.slice(1) : parts;
...
function extractFilesFromCode(code) {
  ...
  const re1 =
    /await\s+(write|edit|writeFile|editFile|deleteFile)\s*\(\s*["'`]([^"'`]+)["'`]/g;
  let m;
  ...
  try {
    ...
    if (files.length === 0) {
      try {
        ...
        if (
          text &&
          /"(write|edit|file_write|file_edit)"/.test(text) &&
          text.includes("file_path")
        ) {
          const reJson1 =
            /"toolName"\s*:\s*"(write|edit|file_write|file_edit|writeFile|editFile)"[^}]*?"file_path"\s*:\s*"([^"]+)"/g;
          const reJson2 =
            /"file_path"\s*:\s*"([^"]+)"[^}]*?"toolName"\s*:\s*"(write|edit|file_write|file_edit)"/g;
          let m;
...
function recordExecutionResults(messageEl, results) {
  ...
  for (const item of results) {
    ...
    const status =
      item.result && item.result.denied
        ? "denied"
...
function renderMetaPanel(messageEl, seconds, tokensEstimate, files) {
  ...
  const normalized = (Array.isArray(effectiveFiles) ? effectiveFiles : [])
    .map(normalizeFileEntry)
    .filter(Boolean)
    .slice(0, 20);
  ...
  let tokensTitle =
    "Токены ответа (серверные, при недоступности — оценка chars / 4)";
  let producedLabel = "Затронуто";
  let producedTitle = "Файлы, затронутые за этот ответ";
  try {
    const tt = t("meta.time.title");
    if (tt && tt !== "meta.time.title") timeTitle = tt;
    const tk = t("meta.tokens.title");
    if (tk && tk !== "meta.tokens.title") tokensTitle = tk;
    const pl = t("meta.produced");
    if (pl && pl !== "meta.produced") producedLabel = pl;
    const pt = t("meta.produced.title");
    if (pt && pt !== "meta.produced.title") producedTitle = pt;
  } catch (_) {}
  let html =
    '<span class="cuckoo-response-meta-item" title="' +
  ...
  if (hasFiles) {
    html +=
      ...
      '<span class="cuckoo-response-meta-produced" title="' +
      escapeHtml(producedTitle) +
      '">' +
      '  <span class="cuckoo-response-meta-produced-label">' +
      escapeHtml(producedLabel) +
      "</span>";
    for (const f of toRender) {
      ...
      html +=
        '<span class="cuckoo-produced-chip" data-path="' +
        escapeHtml(f.path) +
        '" data-op="' +
        escapeHtml(f.op) +
        '" data-status="' +
        ...
        '<span class="cuckoo-produced-chip-icon">' +
        getFileIcon(f.path) +
        "</span>" +
        '<span class="cuckoo-produced-chip-name">' +
        escapeHtml(short) +
        "</span>" +
        "</span>";
    }
        ...
        try {
          ...
          if (!isAbs) {
            ...
            if (projectDir) {
              ...
              console.warn(
                "[Cookie Code] Produced chip: no projectDir to resolve " +
                  rawPath,
              );
              return;
            }
          ...
          window.electronAPI
            ...
            .then((res) => {
              if (res && !res.success)
                console.warn(
                  "[Cookie Code] Produced chip open failed: " +
                    (res.error || "unknown"),
                );
            })
            .catch((err) =>
...
function finishTimer(messageEl) {
  ...
  try {
    if (
      lastServerDelta.value > 0 &&
      (!rec || lastServerDelta.at >= rec.start)
    ) {
...
function getCachedStore() {
  const now = Date.now();
  if (cachedStore && now - cachedStoreTime < STORE_CACHE_TTL)
    return cachedStore;
...
function clearStorage() {
  try {
    ...
    console.error(
      "[Cookie Code] Не удалось очистить мета-данные:",
      err.message,
    );
  }
}
...
function resetForTests() {
  ...
  try {
    if (state) state.showProducedFiles = true;
  } catch (_) {}
  try {
    localStorage.removeItem(STORAGE_KEY);
  } catch (_) {}
  try {
    localStorage.removeItem("cuckoo-results");
  } catch (_) {}
}
