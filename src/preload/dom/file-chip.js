...
/** Обёртка fallback-иконки чипа. */
function svgWrap(inner) {
  return (
    '<svg class="' +
    CHIP_CLASS +
    '__fallback" viewBox="0 0 24 24" aria-hidden="true">' +
    inner +
    "</svg>"
  );
}

/** Базовая иконка «файл» (fallback, если Material-иконка не найдена). */
const FILE_ICON_SVG = svgWrap(
  '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>' +
    '<polyline points="14 2 14 8 20 8"/>',
);

/**
 * Кэш Material-иконок из главного процесса.
 * icons: { fileName: '<svg…>' }
 * fileExtensions: { lua: 'lua', … }  (ext → имя SVG-файла)
 * fileNames: { dockerfile: 'docker', … }
 */
let MATERIAL_ICONS = { icons: {}, fileExtensions: {}, fileNames: {} };

/**
 * Загрузить карту иконок Material Icon Theme из main-процесса.
 * Вызывается один раз при старте. При ошибке — остаются fallback-иконки.
 */
async function loadMaterialIcons() {
  try {
    if (
      !window.electronAPI ||
      typeof window.electronAPI.getFileIcons !== "function"
    ) {
      console.warn(
        "[Cookie Code] file-chip: electronAPI.getFileIcons недоступен — только fallback",
      );
      return;
    }
    const res = await window.electronAPI.getFileIcons();
    if (res && res.success && res.data) {
      MATERIAL_ICONS = {
        icons: res.data.icons || {},
        fileExtensions: res.data.fileExtensions || {},
        fileNames: res.data.fileNames || {},
      };
      console.log(
        "[Cookie Code] file-chip: Material-иконок загружено",
        Object.keys(MATERIAL_ICONS.icons).length,
        "уникальных, расширений:",
        Object.keys(MATERIAL_ICONS.fileExtensions).length,
        "имён:",
        Object.keys(MATERIAL_ICONS.fileNames).length,
      );
      // Пересканируем уже отрендеренные ответы, чтобы применить иконки.
      try {
        scanAll();
      } catch (_) {}
    }
    ...
    console.warn(
      "[Cookie Code] file-chip: loadMaterialIcons error:",
      err.message,
    );
  }
}
...
/**
 * Определить SVG-иконку по пути/имени файла.
 * Сначала ищет по полному имени (fileNames), затем по расширению (fileExtensions).
 * Если ничего не найдено — возвращает fallback.
 * @param {string} filePath
 * @returns {string} SVG-разметка
 */
function getFileIcon(filePath) {
  if (!filePath || typeof filePath !== "string") return FILE_ICON_SVG;
  const name = filePath.replace(/\\/g, "/").split("/").pop().toLowerCase();
  if (!name) return FILE_ICON_SVG;

  const resolve = (iconName) =>
    iconName && MATERIAL_ICONS.icons[iconName]
      ? MATERIAL_ICONS.icons[iconName]
      : null;

  // 1) Точное имя файла (dockerfile, makefile, .gitignore, license и т.п.)
  let svg = resolve(MATERIAL_ICONS.fileNames[name]);
  if (svg) return svg;
  ...
  if (dot > 0) {
    ...
    svg = resolve(MATERIAL_ICONS.fileExtensions[ext]);
    if (svg) return svg;
  }
...
function resolveToAbsolute(p) {
  ...
  if (!projectDir) {
    return {
      abs: null,
      error:
        "Не выбран проект (currentProjectDir пуст) — нельзя разрешить относительный путь",
    };
  }
  ...
  const abs =
    projectDir.replace(/[\\/]+$/, "") + sep + rel.replace(/^[\\/]+/, "");
  return { abs };
}
  ...
  const open = (e) => {
    ...
    try {
      if (
        ...
        console.warn(
          "[Cookie Code] open-path: electronAPI.openPath не доступен (обновите preload)",
        );
        return;
      }
      ...
      if (!resolved.abs) {
        console.warn(
          "[Cookie Code] open-path: " +
            (resolved.error || "не удалось разрешить путь"),
        );
        return;
      }
      window.electronAPI
        .openPath(resolved.abs)
        .then((res) => {
          if (res && !res.success) {
            console.warn(
              "[Cookie Code] open-path: " + (res.error || "не удалось открыть"),
            );
          }
        })
        .catch((err) => {
...
function scanAll(root) {
  ...
  safe("file-chip.scanAll", () => {
    ...
    const codes = scope.querySelectorAll(
      ".ds-markdown code, .ds-markdown-paragraph code",
    );
...
function startWatch() {
  ...
  // Загружаем карту Material-иконок (асинхронно, не блокирует старт).
  loadMaterialIcons();
