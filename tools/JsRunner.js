...
const BOOTSTRAP = [
  ...
  "  };",
  "  globalThis.cityTime = async function () {",
  "    return await __call('city_time', {});",
  "  };",
  ...
  "  };",
  "  globalThis.skillList = async function () {",
  "    return await __call('skill_list', {});",
  "  };",
  "  globalThis.skillLoad = async function (name) {",
  "    return await __call('skill_load', { name: name });",
  "  };",
  "  globalThis.skillExecute = async function (skill, functionName, args) {",
  "    return await __call('skill_execute', { skill: skill, function: functionName, args: args || {} });",
  "  };",
  "  globalThis.askUserQuestion = async function (questions) {",
  ...
  "  };",
  "  globalThis.injectPageJS = async function (code, windowId) {",
  "    return await __call('inject_page_js', { code: code, windowId: windowId });",
  "  };",
  "",
...
class JsRunner {
  ...
  async run(
    ...
    const hostBridge = async (op, argsJson) => {
      if (Date.now() - startTime > deadlineMs) {
        return JSON.stringify({
          success: false,
          error: "JS 脚本执行超时（" + Math.round(deadlineMs / 1000) + " 秒）",
        });
      }
      let args = {};
      try {
        ...
        if (!tool) {
          ...
          try {
            ...
            result = {
              success: false,
              error:
                "工具 " + op + " 执行异常: " + (err.message || String(err)),
            };
          }
        }
      }
    ...
    try {
      Object.setPrototypeOf(hostBridge, null);
    } catch (e) {
      /* 尽力而为 */
    }
    ...
    try {
      ...
      return {
        success: false,
        error: "沙箱初始化失败: " + (err.message || String(err)),
      };
    }
    ...
    try {
      const deadline = new Promise((_resolve, reject) => {
        settleTimer = setTimeout(
          () =>
            reject(
              new Error(
                "JS 脚本执行超时（" + Math.round(deadlineMs / 1000) + " 秒）",
              ),
            ),
          deadlineMs,
        );
      ...
      const ret = await Promise.race([
        script.runInContext(context, { timeout: SYNC_TIMEOUT }),
        deadline,
      ]);
      ...
      let logs = [];
      try {
        const logsJson = vm.runInContext(
      ...
      console.error(
        "[JsRunner] [诊断] 失败代码(JSON转义): " + JSON.stringify(code),
      );
      return {
        success: false,
